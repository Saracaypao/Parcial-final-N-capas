...
public class UserController {
    ...
    @PostMapping
    public ResponseEntity<GeneralResponse> createUser(@Valid @RequestBody UserCreateRequest req) {
        UserCreateRequest dtoConHash = UserCreateRequest.builder()
                .nombre(req.getNombre())
                .correo(req.getCorreo())
                .password(passwordEncoder.encode(req.getPassword()))
                .nombreRol(req.getNombreRol())
                .build();

        UserResponse createdUser = userService.save(dtoConHash);
        return ResponseBuilderUtil.buildResponse(
